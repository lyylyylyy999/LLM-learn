import logging
import pytest
from exercises.task_006_llm_summary.llm_summary import llm_summary, SuccessResult, SummarySettings, LLMError
...
@pytest.mark.parametrize(
        ...
        [
            ...
            ("  ", 300, ValueError, "model 不能是空字符串或者纯空白模型名"),
            ("\n", 300, ValueError, "model 不能是空字符串或者纯空白模型名"),
            ("test", 0, ValueError, "max_output_tokens 不能小于 1"),
            ("test", -300, ValueError, "max_output_tokens 不能小于 1"),
        ]
)
def test_exception_settings(model: str, max_output_tokens: int, exception: type[Exception], match: str) -> None:
...
def test_completed_but_output_text_is_empty(output_text: str, exception: type[Exception]) -> None:
    fake_response = SimpleNamespace(
        status="completed",
    ...
    with pytest.raises(exception, match=f"发生异常，响应 ID: resp_123,状态: completed"):
        llm_summary(
            client=client,
            text="什么是过拟合?",
            summary_settings=settings,
